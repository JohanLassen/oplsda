...
fit_models <- function(x, y, methods = c("glmnet", "ranger", "pls")){
  cat("\nNotice:\nDepending on the data size this process may take 1-60 minutes\n\n")
  trctrl <- caret::trainControl(method = "repeatedcv", number = 5, repeats = 5, verboseIter = T, savePredictions = T, classProbs = T)
  fits   <- as.list(methods) %>% purrr::map(~caret::train(x = x, y=y, method = .x, tuneLength = 3, trControl = trctrl))
  names(fits) <- methods
  return(fits)
}
...
plot_performance <- 
  function(fits){
    ...
    rocdata <- MLeval::evalm(fits, silent = T, showplots = F, gnames = names(fits))$roc$data
